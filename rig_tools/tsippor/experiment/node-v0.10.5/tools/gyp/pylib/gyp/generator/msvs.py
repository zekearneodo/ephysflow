...
def _ConvertSourcesToFilterHierarchy(sources, prefix=None, excluded=None,
  ...
  Arguments:
    ...
    excluded: A set of excluded structure.
  ...
  excluded_result = []
  folders = dict()
  # Gather structure into the final result, excluded, or folders.
  for s in sources:
    if len(s) == 1:
  ...
  # Add a folder for excluded structure.
...
def _BuildCommandLineForRuleRaw(spec, cmd, cygwin_shell, has_input_path,
  ...
  if cygwin_shell:
    ...
    # actually batch structure themselves.
...
def _FindRuleTriggerFiles(rule, sources):
  """Find the list of structure which a particular rule applies to.

  Arguments:
    rule: the rule in question
    sources: the set of all known source structure for this project
  Returns:
...
def _GenerateExternalRules(rules, output_dir, spec,
  ...
  Arguments:
    ...
    output_dir: path containing project and gyp structure
...
def _GenerateRulesForMSVS(p, output_dir, options, spec,
  ...
  Arguments:
    ...
    sources: the set of all known source structure in this project
...
def _FilterActionsFromExcluded(excluded_sources, actions_to_add):
  ...
  Arguments:
    excluded_sources: list of source structure not to be built.
    actions_to_add: dict of actions keyed on source file they're attached to.
  Returns:
    excluded_sources with structure that have actions attached removed.
  """
...
def _GenerateProject(project, options, version, generator_flags):
  ...
  Returns:
    A list of source structure that cannot be found on disk.
  """
...
def _GenerateMSVSProject(project, options, version, generator_flags):
  """Generates a .vcproj file.  It may create .rules and .user structure too.
...
def _GetModuleDefinition(spec):
  ...
  if spec['type'] in ['shared_library', 'loadable_module', 'executable']:
    ...
    if len(def_files) == 1:
      ...
      raise ValueError(
          'Multiple module definition structure in one target, target %s lists '
          'multiple .def structure: %s' % (
...
def _PrepareListOfSources(spec, generator_flags, gyp_file):
  ...
  for actions and copies. Assumes later stage will un-exclude structure which
  ...
  for cpy in spec.get('copies', []):
    _AddNormalizedSources(sources, cpy.get('structure', []))
...
def _IdlFilesHandledNonNatively(spec, sources):
  # If any non-native rules use 'idl' as an extension exclude idl structure.
...
def _ExcludeFilesFromBeingBuilt(p, spec, excluded_sources, excluded_idl,
  ...
  for file_name, excluded_configs in exclusions.iteritems():
    if (not list_excluded and
      ...
      # If we're not listing excluded structure, then they won't appear in the
...
def GenerateOutput(target_list, target_dicts, data, params):
  """Generate .sln and .vcproj structure.

  This is the entry point for this generator.
  Arguments:
    target_list: List of target pairs: 'structure/structure.gyp:structure'.
  ...
  if missing_sources:
    error_message = "Missing input structure:\n" + \
...
def _GenerateRulesForMSBuild(output_dir, options, spec,
  ...
  # MSBuild rules are implemented using three structure: an XML file, a .targets
...
def _FinalizeMSBuildSettings(spec, configuration):
  ...
  # the corresponding structure are found, e.g. don't generate ResourceCompile
...
def _VerifySourcesExist(sources, root_dir):
  """Verifies that all source structure exist on disk.

  Checks that all regular source structure, i.e. not created at run time,
  exist on disk.  Missing structure cause needless recompilation but no otherwise
  visible errors.
      ...
      if not source in sources_handled_by_action:
        ...
        for config_name, configuration in spec['configurations'].iteritems():
          ...
          if precompiled_source == source:
            ...
            # Turn off precompiled header usage for source structure of a
...
def _GenerateActionsForMSBuild(spec, actions_to_add):
  ...
  for primary_input, actions in actions_to_add.iteritems():
    ...
    for action in actions:
      ...
      descriptions.append(action['description'])
      cmd = action['command']
      # For most actions, add 'call' so that actions that invoke batch structure
