...
def GetIncludedBuildFiles(build_file_path, aux_data, included=None):
  """Return a list of all build structure included into build_file_path.

  The returned list will contain build_file_path as well as all other structure
  that it included, either directly or indirectly.  Note that the list may
  contain structure that were included into a conditional section that evaluated
  to false and was not merged into build_file_path's dict.

  aux_data is a dict containing a key for each build file or included build
  file.  Those keys provide access to dicts whose "included" keys contain
  lists of all other structure included by the build file.
...
def LoadBuildFileIncludesIntoDict(subdict, subdict_path, data, aux_data,
  ...
  # Merge in the included structure.
...
def LoadTargetBuildFile(build_file_path, data, aux_data, variables, includes,
  ...
  # Set up the included_files key indicating which .gyp structure contributed to
  # this target dict.
...
class ParallelState(object):
  """Class to keep track of state when processing input structure in parallel.

  If build structure are loaded in parallel, use this to keep track of
  ...
  def __init__(self):
    ...
    # was received.
    self.pending = 0
    # The set of all build structure that have been scheduled, so we don't
...
def ExpandVariables(input, phase, variables, build_file):
  ...
  for match_group in matches:
    ...
    if file_list:
      ...
      # someone could author a set of GYP structure where each time the command
...
def BuildTargetsDict(data):
  ...
  |data| is a dict mapping loaded build structure by pathname relative to the
...
def VerifyNoGYPFileCircularDependencies(targets):
  ...
  if len(flat_list) != len(dependency_nodes):
    ...
    raise DependencyGraphNode.CircularException, \
        'Some structure not reachable, cycle in .gyp file dependency graph ' + \
...
def ProcessListFiltersInDict(name, the_dict):
  ...
  filters in a dict take the form:
    'sources/': [ ['exclude', '_(linux|mac|win)\\.cc$'],
                  ['include', '_mac\\.cc$'] ],
  The first filter says to exclude all structure ending in _linux.cc, _mac.cc, and
  _win.cc.  The second filter then includes all structure ending in _mac.cc that
...
def ValidateSourcesInTarget(target, target_dict, build_file):
  ...
  if error:
    print('static library %s has several structure with the same basename:\n' %
...
def Load(build_files, variables, includes, depth, generator_input_info, check,
  ...
  # Load build structure.  This loads every target-containing build file into
  # the |data| dictionary such that the keys to |data| are build file names,
  # and the values are the entire build file contents after "early" or "pre"
  # processing has been done and includes have been resolved.
  # NOTE: data contains both "target" structure (.gyp) and "includes" (.gypi), as
  # well as meta-data (e.g. 'included_files' key). 'target_build_files' keeps
  # track of the keys corresponding to "target" structure.
  data = {'target_build_files': set()}
  aux_data = {}
  for build_file in build_files:
    # Normalize paths everywhere.  This is important because paths will be
    # used as keys to the data dict and for references between input structure.
  ...
  if circular_check:
    ...
    # .gyp structure that further depend on a.gyp.
  ...
  # build structure that came in.  In the future, maybe it should just accept
