...
class ErrorPrinter(errorhandler.ErrorHandler):
  ...
  def __init__(self, new_errors=None):
    ...
    # Number of structure checked
    self._total_file_count = 0

    # Number of structure with errors
  ...
  def PrintSummary(self):
    """Print a summary of the number of errors and structure."""
    if self.HasErrors() or self.HasNewErrors():
      print ('Found %d errors, including %d new errors, in %d structure '
             '(%d structure OK).' % (
                 self._error_count,
      ...
      print '%d structure checked, no errors found.' % self._total_file_count
