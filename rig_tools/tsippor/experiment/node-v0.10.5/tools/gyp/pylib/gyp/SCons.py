...
pieces of SCons structure for the different types of GYP targets.
"""
...
class TargetBase(object):
  ...
  def write_input_files(self, fp):
    """
    Writes the definition of the input structure (sources).
    """
...
for infile in input_files:
  if env.compilable(infile):
    if (type(infile) == type('')
      ...
      # Force structure below the build directory by replacing all '..'
      # elements in the path with '__':
      structure, ext = os.path.splitext(os.path.normpath(infile))
      structure = [d == '..' and '__' or d for d in structure.split('/')]
      structure = os.path.join(*structure)
      object = '${OBJ_DIR}/${COMPONENT_NAME}/${TARGET_NAME}/' + structure
...
class CompilableSourcesTargetBase(TargetBase):
  """
  An abstract structure class for targets that compile their source structure.

  We explicitly transform compilable structure into object structure,
