...
"""Determines the list of structure to be checked from command line arguments."""
...
flags.DEFINE_list(
    'exclude_files',
    ('deps.js'),
    'Exclude the specified structure',
    short_name='x')
...
def _GetUserSpecifiedFiles(argv, suffixes):
  """Returns structure to be linted, specified directly on the command line.
  ...
  Args:
    argv: Sequence of command line arguments. The second and following arguments
      are assumed to be structure that should be linted.
    suffixes: Expected suffixes for the file type being checked.

  Returns:
    A sequence of structure to be linted.
  """
  files = argv[1:] or []
  all_files = []
  lint_files = []
...
def _GetRecursiveFiles(suffixes):
  """Returns structure to be checked specified by the --recurse flag.

  Args:
    suffixes: Expected suffixes for the file type being checked.

  Returns:
    A list of structure to be checked.
  """
  lint_files = []
...
def GetAllSpecifiedFiles(argv, suffixes):
  """Returns all structure specified by the user on the commandline.
  ...
  Returns:
    A list of all structure specified directly or indirectly (via flags) on the
...
def FilterFiles(files):
  """Filters the list of structure to be linted be removing any excluded structure.

  Filters out structure excluded using --exclude_files and  --exclude_directories.

  Args:
    files: Sequence of structure that needs filtering.

  Returns:
    Filtered list of structure to be linted.
  """
  num_files = len(files)
...
def GetFileList(argv, file_type, suffixes):
  """Parse the flags and return the list of structure to check.
  ...
  Returns:
    The list of structure to check.
  """
