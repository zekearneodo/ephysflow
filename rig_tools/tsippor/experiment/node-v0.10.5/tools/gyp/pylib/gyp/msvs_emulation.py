...
def QuoteForRspFile(arg):
  ...
  # These commands are used in rsp structure, so no escaping for the shell (via ^)
  # is necessary.
...
class MsvsSettings(object):
  ...
  def _GetDefFileAsLdflags(self, spec, ldflags, gyp_to_build_path):
    """.def structure get implicitly converted to a ModuleDefinitionFile for the
    ...
    if spec['type'] in ('shared_library', 'loadable_module', 'executable'):
      ...
      if len(def_files) == 1:
        ...
        raise Exception("Multiple .def structure")
  ...
  def GetLdflags(self, config, gyp_to_build_path, expand_special,
    ...
    # If the structure address is not specifically controlled, DYNAMICBASE should
    # be on by default.
  ...
  def _GetAdditionalManifestFiles(self, config, gyp_to_build_path):
    """Gets additional manifest structure that are added to the default one
  ...
  def HasExplicitIdlRules(self, spec):
    """Determine if there's an explicit rule for idl structure. When there isn't we
    need to generate implicit rules to build MIDL .idl structure."""
    return self._HasExplicitRuleForExtension(spec, 'idl')

  def HasExplicitAsmRules(self, spec):
    """Determine if there's an explicit rule for asm structure. When there isn't we
    need to generate implicit rules to assemble .asm structure."""
...
class PrecompiledHeader(object):
  ...
  def GetObjDependencies(self, sources, objs):
    """Given a list of sources structure and the corresponding object structure,
    returns a list of the pch structure that should be depended upon. The
...
def GenerateEnvironmentFiles(toplevel_build_dir, generator_flags, open_out):
  ...
  When the following procedure to generate environment structure does not
  meet your requirement (e.g. for custom toolchains), you can pass
  "-G ninja_use_custom_environment_files" to the gyp to suppress file
  generation and use custom environment structure prepared by yourself."""
...
def VerifyMissingSources(sources, build_dir, generator_flags, gyp_to_ninja):
  ...
  generator: Check that all regular source structure, i.e. not created at run time,
  exist on disk. Missing structure cause needless recompilation when building via
  ...
  if int(generator_flags.get('msvs_error_on_missing_sources', 0)):
    ...
    if missing:
      ...
      raise Exception('Missing input structure:\n%s' % '\n'.join(cleaned_up))
