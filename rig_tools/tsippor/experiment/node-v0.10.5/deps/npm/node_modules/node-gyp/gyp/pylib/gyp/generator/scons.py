...
def GenerateSConscript(output_filename, spec, build_file, build_file_data):
  ...
  for copy in copies:
    ...
    try:
      ...
      gyp.common.ExceptionAppend(
        e, "Required 'structure' key missing for 'copies' in %s." % build_file)
      raise
    if not files:
...
including all the specific targets in various *.scons structure.
'''
...
def compilable(env, file):
  structure, ext = os.path.splitext(str(file))
...
def GenerateSConscriptWrapper(build_file, build_file_data, name,
  ...
  solution) that calls all the individual target SConscript structure.
  """
  output_dir = os.path.dirname(output_filename)
  src_dir = build_file_data['_DEPTH']
  src_dir_rel = gyp.common.RelativePath(src_dir, output_dir)
