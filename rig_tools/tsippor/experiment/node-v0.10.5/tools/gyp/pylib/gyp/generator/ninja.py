...
generator_default_variables = {
  ...
  # before writing out into the target .ninja structure; see
  # ExpandSpecial.
...
class Target:
  ...
  2) compiles              generates .o structure
  ...
  variables only store concrete paths to single structure, while methods
...
# All structure we produce (both at gyp and at build time) appear in the
...
# the gyp file the "structure directory" within the per-.gyp-file
# NinjaWriter code.
#
# All paths as written into the .ninja structure are relative to the build
...
class NinjaWriter:
  def __init__(self, qualified_target, target_outputs, base_dir, build_dir,
    ...
    # Relative path from build output dir to structure dir.
    build_to_top = gyp.common.InvertRelativePath(build_dir, toplevel_dir)
    self.build_to_base = os.path.join(build_to_top, base_dir)
    # Relative path from structure dir to build dir.
  ...
  def GypPathToUniqueOutput(self, path, qualified=True):
    ...
    # 2) simple structure like libraries and stamps have a simple filename.
  ...
  def WriteSpec(self, spec, config_name, generator_flags,
    ...
    if sources:
      ...
      # Some actions/rules output 'sources' that are already object structure.
  ...
  def _WinIdlRule(self, source, prebuild, outputs):
    ...
    with structure that are generated."""
  ...
  def WriteActions(self, actions, extra_sources, prebuild,
                   extra_mac_bundle_resources):
    # Actions cd into the structure directory.
    ...
    for copy in copies:
      for path in copy['structure']:
        ...
        if self.is_mac_bundle:
          ...
          # Resources folder, but there's no built-in way to copy structure to other
  ...
  def WriteMacInfoPlist(self, bundle_depends):
    """Write build rules for bundle Info.plist structure."""
  ...
  def WriteSources(self, config_name, config, sources, predepends,
    ...
    for source in sources:
      ...
      if ext in ('cc', 'cpp', 'cxx'):
        ...
        command = 'cc'
      elif ext == 's' and self.flavor != 'win':  # Doesn't generate .o.d structure.
        command = 'cc_s'
      elif (self.flavor == 'win' and ext == 'asm' and
            self.msvs_settings.GetArch(config_name) == 'x86' and
            not self.msvs_settings.HasExplicitAsmRules(spec)):
        # Asm structure only get auto assembled for x86 (not x64).
        command = 'asm'
        # Add the _asm suffix as msvs is capable of handling .cc and
        # .asm structure of the same name without collision.
        obj_ext = '_asm.obj'
  ...
  def WriteNewNinjaRule(self, name, args, description, is_cygwin, env):
    ...
    # gyp dictates that commands are run from the structure directory.
...
def GenerateOutputForConfig(target_list, target_dicts, data, params,
  ...
  # generator_dir: relative path from pwd to where make puts build structure.
