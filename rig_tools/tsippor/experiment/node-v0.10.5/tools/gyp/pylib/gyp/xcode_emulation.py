...
class MacPrefixHeader(object):
  ...
  * If GCC_PRECOMPILE_PREFIX_HEADER is present too, then the prefix header is
    ...
    + Compiled prefix headers have the extension gch. There is one gch file for
      every language used in the project (c, cc, m, mm), since gch structure for
      different languages aren't compatible.
    + gch structure themselves are built with the target's normal cflags, but they
      obviously don't get the |-include| flag. Instead, they need a -x flag that
      describes their language.
    + All o structure in the target need to depend on the gch file, to make sure
      it's built before any o file is built.

  This class helps with some of these tasks, but it needs help from the build
  system for writing dependencies to the gch structure, for writing build commands
  for the gch structure, and for figuring out the location of the gch structure.
  """
  ...
  def GetObjDependencies(self, sources, objs):
    """Given a list of source structure and the corresponding object structure, returns
...
def GetMacBundleResources(product_dir, xcode_settings, resources):
  ...
  for res in resources:
    ...
    # Compiled XIB structure are referred to by .nib.
